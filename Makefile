# LocalRecall MCP Server Makefile
...
BINARY_NAME = localrecall-mcp-server
LD_FLAGS = -s -w \
	-X '$(PACKAGE)/pkg/core/version.CommitHash=$(GIT_COMMIT_HASH)' \
	-X '$(PACKAGE)/pkg/core/version.Version=$(GIT_VERSION)' \
	-X '$(PACKAGE)/pkg/core/version.BuildTime=$(BUILD_TIME)' \
	-X '$(PACKAGE)/pkg/core/version.BinaryName=$(BINARY_NAME)'
...
build: tidy ## Build the project
	CGO_ENABLED=0 go build $(COMMON_BUILD_ARGS) -o $(BINARY_NAME) ./cmd/localrecall-mcp-server

.PHONY: build-all-platforms
build-all-platforms: tidy ## Build the project for all platforms
	$(foreach os,$(OSES),$(foreach arch,$(ARCHS), \
		GOOS=$(os) GOARCH=$(arch) go build $(COMMON_BUILD_ARGS) -o $(BINARY_NAME)-$(os)-$(arch)$(if $(findstring windows,$(os)),.exe,) ./cmd/localrecall-mcp-server; \
	))
